<x-app-layout>
    ...
    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-10 gap-6">
        <div>
            <h2 class="text-3xl font-black text-slate-900 tracking-tight">Daftar Pengeluaran</h2>
            <p class="text-sm text-slate-500 mt-1 font-medium italic">"Setiap pengeluaran adalah investasi dalam kenyamanan, kelola dengan bijak."</p>
        </div>
        <a href="{{ route('expenses.create') }}" class="group inline-flex items-center justify-center px-6 py-3.5 bg-rose-600 border border-transparent rounded-[1.2rem] font-bold text-xs text-white transition-all hover:bg-rose-700 focus:outline-none focus:ring-4 focus:ring-rose-500/20 shadow-xl shadow-rose-600/20 active:scale-95 uppercase tracking-widest">
    ...
    <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-900/5 border border-slate-100 p-8 mb-8">
        <form method="GET" action="{{ route('expenses.index') }}" class="space-y-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center pt-2 gap-4 border-t border-slate-50 mt-2">
                <div class="flex items-center gap-4 w-full md:w-auto">
                    ...
                    @if(request()->hasAny(['search', 'category_id', 'start_date', 'end_date']))
                        <a href="{{ route('expenses.index') }}" class="flex items-center gap-2 px-5 py-3.5 bg-slate-100 text-slate-500 rounded-2xl text-xs font-bold uppercase tracking-widest hover:bg-rose-50 hover:text-rose-600 transition-all group">
                ...
                <div class="flex gap-4">
                    <div class="flex flex-col items-end">
                        ...
                        <span class="text-lg font-black text-rose-600">Rp {{ number_format($expenses->sum('amount'), 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
        </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100/50">
                    ...
                    <tr class="hover:bg-rose-50/30 transition-all duration-300 group {{ $expense->amount > 1000000 ? 'bg-rose-50/10' : '' }}">
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 flex flex-col items-center justify-center text-slate-500 group-hover:bg-white group-hover:border-rose-100 group-hover:text-rose-600 transition-all shadow-sm">
                                    <span class="text-[10px] font-black uppercase leading-tight tracking-tighter">{{ $expense->date->format('M') }}</span>
                        ...
                        <td class="px-8 py-6">
                            <div class="flex flex-col">
                                <span class="text-base font-black text-rose-600">Rp {{ number_format($expense->amount, 0, ',', '.') }}</span>
                                @if($expense->amount > 1000000)
                                    <span class="text-[8px] font-black text-rose-400 uppercase tracking-tighter mt-0.5">⚠️ Pengeluaran Besar</span>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex justify-end gap-3 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all transform translate-x-2 sm:group-hover:translate-x-0">
                                <a href="{{ route('expenses.edit', $expense) }}" class="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:bg-white border border-transparent hover:border-indigo-100 rounded-xl transition-all shadow-none hover:shadow-lg hover:shadow-indigo-500/10" title="Edit Data">
                                ...
                                <form action="{{ route('expenses.destroy', $expense) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus permanen data pengeluaran ini?');">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="4" class="px-8 py-24 text-center">
                            <div class="flex flex-col items-center justify-center">
                                <div class="w-24 h-24 bg-slate-50 text-rose-100 rounded-full flex items-center justify-center mb-6 shadow-inner">
                                ...
                                <h3 class="text-lg font-black text-slate-900 mb-1">Pengeluaran Kosong</h3>
                                <p class="text-sm text-slate-500 mb-8 font-medium italic">"Mungkin filter pencarianmu terlalu ketat?"</p>
                                <a href="{{ route('expenses.index') }}" class="px-6 py-3 bg-rose-50 text-rose-600 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-rose-600 hover:text-white transition-all shadow-sm">Reset Filter</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
