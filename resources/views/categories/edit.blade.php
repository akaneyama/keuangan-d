<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-[2.5rem] p-10 shadow-xl border border-slate-100 relative overflow-hidden">
            ...
            <form action="{{ route('categories.update', $category) }}" method="POST" class="space-y-8 relative z-10">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $category->name) }}"
                           ...
                           placeholder="Misal: Makanan, Gaji..." required autofocus>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <label class="relative cursor-pointer group">
                            <input type="radio" name="type" value="income" class="sr-only peer" {{ old('type', $category->type) == 'income' ? 'checked' : '' }}>
                        ...
                        <label class="relative cursor-pointer group">
                            <input type="radio" name="type" value="expense" class="sr-only peer" {{ old('type', $category->type) == 'expense' ? 'checked' : '' }}>
                ...
                <div class="pt-6 flex flex-col sm:flex-row items-center gap-4">
                    ...
                    <button type="submit" class="w-full sm:flex-[2] px-8 py-4 bg-indigo-600 text-white font-black uppercase tracking-widest text-[10px] rounded-[1.2rem] hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-600/20 active:scale-95 group flex items-center justify-center gap-2">
                        Perbarui Kategori
                        <svg class="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M4 4h16v16H4zM4 4l16 16M4 20L20 4" /></svg>
                    </button>
                </div>
            </form>
        </div>
